...
-- C:\Program Files (x86)\World of Warcraft\_classic_\Interface\AddOns\ThreatClassic2\core\core.lua
-- The tweaks are marked with the text "dominic"
-- The idea was to show absolute threat rather than relative threat.
-- I am reverting to relative threat, and so this is just a record
-- of what the tweaks were.
...
function TC2:UpdateThreatBars()
    ...
    local denom = 10000  -- dominic
    local doneFirstBar = false  -- dominic
    ...
    for i = 1, C.bar.count do
        ...
        if data and data.threatValue > 0 then
            ...
            -- bar:SetValue(data.threatPercent)  -- dominic
            -- start dominic
            if not doneFirstBar then
                if data.threatValue > 2 * denom then
                    denom = 4 * denom
                elseif data.threatValue > denom then
                    denom = 2 * denom
                end
                doneFirstBar = true
            end
            -- end dominic
            bar:SetValue(data.threatValue / denom * 100)  -- dominic
